...
【历史参考案例】（按相关度从高到低排列，案例1是最佳匹配）：
{few_shot_context}

【编写策略】：
1. **优先复用案例1**：案例1与当前需求最相似。如果它覆盖了用户要求的列，请直接复用它的表关联结构、计算逻辑和 UNION ALL 模式，只修改 WHERE 条件和过滤参数。不要重写轮子。
2. **覆盖所有列**：仔细检查用户需求中的每一列，确保 SQL 中都有对应的 SELECT 表达式。被用户点名的列一个都不能少。
3. 数据隔离：必须使用 `org_no LIKE 'xxx%'` 进行模糊匹配。
4. 性能：严禁在庞大的流水表上无条件进行外层 LEFT JOIN 和全局聚合。通过关联条件限制扫描行数。
5. 版本限制（极其重要）：当前数据库为 MySQL 5.7，绝对禁止使用 `WITH` (CTE) 语法。使用传统的嵌套子查询（派生表）。
6. 请使用 `<think>` 标签梳理逻辑，最后输出 ```sql ... ``` 代码块。"""),
        ("human", "{question}")
    ])
    return prompt | llm | StrOutputParser()
