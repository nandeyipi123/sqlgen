...
SELECT
	...
	( SELECT NAME FROM ac_org o WHERE o.id = c.org_no ) 供电单位,
	...
	( SELECT NAME FROM m_r_sect rr WHERE id = c.mr_sect_no AND effect_flag = 1 ) 抄表段名称,
	...
	FROM
		...
		m_r_oper_activity dd

	WHERE

		dd.mr_sect_no = c.mr_sect_no
	...
	( SELECT NAME FROM m_p_code WHERE code_type = 'statusCode' AND VALUE = c.status_code ) 用户状态,
...
select
  ...
  (select name from ac_org where id=a.org_no LIMIT 1) 单位名称,
  ...
  (select name from m_p_code m where m.code_type ='readTypeCode' and m.value=b.read_type_code LIMIT 1) 示数类型,
  ...
  (select name from m_p_code cd where cd.code_type='mrModeCode' and cd.value=b.actual_mode LIMIT 1) 抄表方式,

  (select name from m_p_code cd where cd.code_type='mrStatusCode' and value=b.mr_status_code LIMIT 1) 抄表状态,
...
WHERE para.calc_id=a.calc_id AND para.mp_id =mp.id ORDER BY mp.mp_sn DESC LIMIT 1)),(SELECT line_name FROM m_g_line WHERE line_no={线路编号})) '线路名称',
...
-- 文件名：公安-电力用户信息
select a.cons_no '用户编号',a.cons_name '用户名称',(select cert_name from m_c_cert  where id=b.cert_id) '证件类型',(select cert_no from m_c_cert  where id=b.cert_id)  '证件号码',

(select ifnull(mobile,office_tel) from m_c_contact where id=c.contact_id) '联系电话',a.elec_addr '用电地址',(select name from ac_org where id=a.org_no) '供电单位',null '缴费号',a.mr_sect_no '抄表段编号',

(select name from m_p_code where code_type='elecTypeCode' and value=a.elec_type_code) '用电类别',(select name from m_p_code where code_type='statusCode' and value=a.status_code) '用户状态',

(select name from m_p_code where code_type='psVoltCode' and value=a.volt_code)'供电电压',a.build_date '开户时间'
...
(select cert_no from m_c_cert  where id=c.cert_id)  '证件号码',

(select ifnull(mobile,office_tel) from m_c_contact where id=f.contact_id) '联系电话',

b.elec_addr '用电地址',

(select name from m_p_code where code_type='elecTypeCode' and value=b.elec_type_code) '用电类别',

a.t_pq '总结算电量',null '缴费号',b.orgn_cons_no '原户号', e.prc_value '电价',

 e.prc_name '电价名称',

 (select name from m_p_code where code_type='psVoltCode' and value=b.volt_code)'供电电压',

 a.rcvbl_amt '总结算电费',a.release_date '计算日期', a.release_date '发行日期',

f.rcved_ym '缴费月份',

(select name from ac_org where id=b.org_no) '供电单位',
...
SELECT
  ...
  ( SELECT NAME FROM m_p_code WHERE code_type = 'custSortCode' AND VALUE = c.cons_sort_code ) 用户分类,
	...
	( SELECT NAME FROM m_p_code WHERE code_type = 'elecTypeCode' AND VALUE = c.elec_type_code ) 用电类别,
...
-- 文件名：市场化交易品种初版
SELECT (SELECT name FROM ac_org WHERE id = b.org_no LIMIT 1) 单位,
...
-- 文件名：一户多人口阶梯电量查询
SELECT (select o.name from ac_org o where o.code = a.org_no)         AS '单位',
...
select
 ...
 (select name from ac_org where id=mp.org_no LIMIT 1) 单位名称,

 mp.mp_no 计量点编号,

 mp.mp_name 计量名称,

 (SELECT name from m_p_code cd where cd.code_type='mpSortCode' and cd.value=mp.type_code) 计量点分类,
 ...
 (SELECT CONCAT_WS( '|', id, NAME ) FROM m_r_sect r WHERE r.id = mp.mr_sect_no ) 抄表段,
 ...
 (SELECT name from m_p_code cd where cd.code_type='chkunitSort' and cd.value=ch.chkunit_type_code) 考核单元分类,
...
ym 电费年月,prc_code 电价,prc_name 电价名称,(SELECT name FROM m_p_code WHERE code_type ="noteTypeCode" AND VALUE = note_type_code LIMIT 1) 票据类型
...
select
	...
	(select name from m_r_sect where id=a.mr_sect_no limit 1) 抄表段名称
...
-- 文件名：渠道收费统计
select (select name from ac_org where id=ifnull(c.code,p.dept_no)) 单位,d.name 渠道,f.name 付款方式,sum(ifnull(p.rcv_amt,0)) 金额, count(*) 笔数
...
SELECT
	...
	(select name from ac_org where code =a.org_no) 单位,
	...
	( SELECT CONCAT_WS( '|', id, NAME ) FROM m_r_sect r WHERE r.id = a.mr_sect_no ) 抄表段信息,
	...
	(select name from m_p_code where code_type='ctlMode' and value=a.ctl_mode ) 电费类型,

	(select name from m_p_code where code_type='paTypeCode' and value=a.amt_type) 电费类别
...
(SELECT name from ac_org o where o.id=substring(a.org_no,1,7)) 分公司,

a.org_no 单位编码,

(SELECT name from ac_org o where o.id=a.org_no ) 供电所,
...
(select name from m_p_code where code_type ='tradeCode'and value= b.trade_type_code) 行业类别,

max(b.prc_code) 电价码,

(select prc.cat_prc_name from m_e_cat_prc prc where prc.prc_code = b.prc_code and para_vn = '12019070101') 电价名称,

sum(b.t_settle_pq) 营销总电量,

sum(b.t_amt) 营销总电费,

(-1)*sum((select t_amt from m_e_cons_prc_amt d where d.prc_code =b.prc_code and d.calc_id =b.calc_id and d.prc_snap_id = b.prc_snap_id and d.amt_type='99')) 优惠电费
...
(-1)*sum((select t_amt from m_e_cons_prc_amt_arc d where d.prc_code =b.prc_code and d.calc_id =b.calc_id and d.prc_snap_id = b.prc_snap_id and d.amt_type='99' and d.ym = a.ym)) 优惠电费
...
(select prc.cat_prc_name from m_e_cat_prc prc where prc.prc_code = b.prc_code and prc.para_vn = b.para_vn) 电价名称,
...
SELECT
	...
	( SELECT NAME FROM m_p_code WHERE code_type = 'elecTypeCode' AND VALUE = c.elec_type_code ) 用电性质,
	...
	WHERE
		...
		cat.para_vn = '40500000021'

		AND cat.prc_code = p.prc_code
...
( SELECT NAME FROM m_p_code WHERE code_type = 'ctlMode' AND VALUE = c.ctl_mode ) 档案费控分类,

( SELECT NAME FROM m_p_code WHERE code_type = 'prepayCode' AND VALUE = b.bal_type )   余额类型
...
SELECT
	...
	( SELECT NAME FROM ac_org WHERE CODE = c.org_no ) 单位,
	...
	( SELECT CONCAT_WS( '|', id, NAME ) FROM m_r_sect r WHERE r.id = c.mr_sect_no ) 抄表段信息,
...
SELECT
	...
	(select name from m_p_code where  code_type ='prepayCode' and value=p.bal_type) 余额类型
...
select distinct o.name 单位名称,cons.id 用户ID,
  ...
  (SELECT CONCAT_WS( '|', id, NAME ) FROM m_r_sect r WHERE r.id = cons.mr_sect_no ) 抄表段,
...
-- 文件名：工商业电价明细核对
select (select `name` from ac_org where id=left(a.org_no,7)) 公司名称
  ...
  ,(select sum(pl_prc) from m_e_add_pl_prc where det_id=b.id and pl_code='10000') 农网还贷

  ,(select sum(pl_prc) from m_e_add_pl_prc where det_id=b.id and pl_code='20000') 水利基金

  ,(select sum(pl_prc) from m_e_add_pl_prc where det_id=b.id and pl_code='30000') 库区移民基金

  ,(select sum(pl_prc) from m_e_add_pl_prc where det_id=b.id and pl_code='40000') 可再生能源

  ,(select sum(pl_prc) from m_e_add_pl_prc where det_id=b.id and pl_code='40001') 可再生能源（灾区）

  ,(select sum(pl_prc) from m_e_add_pl_prc where det_id=b.id and pl_code='90000') 电能替代过网费

  ,(select sum(pl_prc) from m_e_add_pl_prc where det_id=b.id and ifnull(prc_io_flag,'')<>'03') 代征合计

  ,(select sum(trans_dist_prc) from m_e_cat_prc_det_items where para_vn=a.para_vn and prc_id=a.id and det_id=b.id) 输配合计

  ,(select sum(agent_prc) from m_e_cat_prc_det_items where para_vn=a.para_vn and prc_id=a.id and det_id=b.id) 代理合计

  ,(select sum(content3) from m_e_cat_prc_det_items where det_id=b.id) 上网环节线损合计

  ,(select sum(content4) from m_e_cat_prc_det_items where det_id=b.id) 系统运行费合计
...
SELECT
	...
	( SELECT NAME FROM m_p_code WHERE code_type = 'tradeCode' AND VALUE = a.trade_code ) 行业类别,
...
SELECT
	...
	(SELECT NAME FROM ac_org WHERE CODE = cons.org_no) 单位名称,

	(select NAME from m_p_code a where a.code_type='ruralConsCode' and cons.rural_cons_code=a.`value`) 城农网标志,
...
SELECT
	...
	( SELECT CONCAT_WS( '|', id, NAME ) FROM m_r_sect r WHERE r.id = a.mr_sect_no ) 抄表段,
	...
	( SELECT NAME FROM m_p_code WHERE code_type = 'prepayCode' AND VALUE = b.card_bal ) 余额类型,
...
SELECT
	...
	(
		...
		WHERE
			...
			pc.`value` = a.note_type_code

		AND pc.code_type = 'noteTypeCode'
...
-- 文件名：应实未查询
select (select name from ac_org where code ={供电单位}) 供电单位,ys.*,b.实收电费,c.欠费
...
SELECT
	...
	( SELECT NAME FROM ac_org WHERE CODE = c.org_no ) 用户单位,

	( SELECT NAME FROM ac_org WHERE CODE = ( SELECT l.org_no FROM m_g_line l WHERE id = a.line_id ) ) 线路单位,

-- 	a.line_id,

	( SELECT l.line_no FROM m_g_line l WHERE id = a.line_id ) 线路编号,

	( SELECT l.line_name FROM m_g_line l WHERE id = a.line_id ) 线路名称,

-- 	a.tg_id,

	( SELECT g.tg_no FROM m_g_tg g WHERE id = a.tg_id ) 台区编号,

	( SELECT g.tg_name FROM m_g_tg g WHERE id = a.tg_id ) 台区名称,
...
SELECT
	...
	( SELECT NAME FROM ac_org WHERE CODE = a.org_no ) 用户单位,
...
SELECT DISTINCT
			...
			(select name from ac_org where id = cons.org_no) 单位,

    	(select `name` from m_r_sect WHERE id = cons.mr_sect_no) 抄表段,
			...
			(select `name` from m_p_code where code_type='elecTypeCode' and `value`= cons.elec_type_code)  用电类别,
			...
			(select `name` from m_p_code where code_type='custSortCode' and `value`= cons.cons_sort_code) 用户分类,

			(select `name` from m_p_code where code_type='statusCode' and `value`= cons.status_code) 用户状态
...
-- 文件名：上网电量结算汇总
SELECT (SELECT name FROM ac_org WHERE id = amt.org_no) 管理单位,
...
SELECT
	...
	( SELECT NAME FROM ac_org o WHERE o.id = rbl.org_no ) 供电单位,
...
SELECT
  ...
  (select name from ac_org where code =fox.org_no ) 用户单位,
...
fox.mr_sect_no 抄表段,
	...
	(select name from m_p_code p where code_type='settleFlag' and value=fox.settle_flag) 发行结清标识,

	sf.CHARGE_EMP_NO 收费人员,

	(select name from m_p_code p where code_type='payMode' and value=sf.pay_mode)  缴费方式,

	(select name from m_p_code p where code_type='settleMode' and value=sf.settle_mode)  结算方式
...
SELECT
 ...
 (select name from m_p_code p where code_type='ctlMode'  and value=c.ctl_mode) 费控类型,

 (select name from m_p_code p where code_type='custSortCode'  and value=c.cons_sort_code) 用户分类,
